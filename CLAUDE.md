...
nimble local             # Build and copy binaries to root directory for convenience
...
- **`nimble local`**: Builds the project using `nimble build` and copies the `ntalk` and `ntalkc` binaries to the root directory for easy access
